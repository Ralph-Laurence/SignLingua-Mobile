...
public class LearnerClassroomsActivity
        extends AppCompatActivity
        implements LearnerClassroomsJsBridge.LearnerClassroomsJsBridgeListener
{
    private WebView m_webView;
    private LearnerManagementApiService learnerMgtApiService;
    private String lastLoadedUrl = "";
    private final String JS_BRIDGE_NAME = "LearnerClassroomsJsBridge";
        ...
        m_webView.setWebViewClient(new WebViewClient()
        {
            ...
            public void onPageFinished(WebView view, String url)
            {
                ...
                if (!url.equals(lastLoadedUrl))
                {
                    ...
                    runOnUiThread(() -> fetchClassrooms());
                }
            }
        });

        // Add the login bridge defined in a separate file.
        m_webView.addJavascriptInterface(new LearnerClassroomsJsBridge(this), JS_BRIDGE_NAME);

        m_webView.loadUrl("file:///android_asset/pages/learner_classrooms.html");
    ...
    {
        runOnUiThread(() -> {
            Intent intent = new Intent(LearnerClassroomsActivity.this, LearnerHomeActivity.class);
    ...
    {
        learnerMgtApiService.fetchClassrooms().enqueue(new Callback<>()
        {
            @Override
            public void onResponse(Call<LearnerClassroomsResponse> call, Response<LearnerClassroomsResponse> response)
            {
                ...
                try
                {
                    ...
                    Log.d("MINE", "Classrooms -> " + jsonResponse);
                    // Pass JSON string safely to WebView
                    m_webView.evaluateJavascript("javascript:renderClassrooms(`" + jsonResponse + "`);", null);
            ...
            @Override
            public void onFailure(Call<LearnerClassroomsResponse> call, Throwable t)
            {
