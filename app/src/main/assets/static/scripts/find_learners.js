...
$(document).ready(() => {
    ...
    fabClearFilters.on('click', () => {
        ...
        if (window.FindLearnerJsBridge)
        {
            window.FindLearnerJsBridge.onFindLearners(null, page, selectedDisabilityFilter);
        }
    });
    ...
    $('#btn-back').on("click", function () {
        ...
        if (window.FindLearnerJsBridge) {
            window.FindLearnerJsBridge.onGoBack();
        }
    });
    ...
    $(document).on("click", '.page-link', function (e)
    {
        ...
        if (page && window.FindLearnerJsBridge)
        {
            window.FindLearnerJsBridge.onFindLearners(null, page, null);
        }
    })
    ...
    .on('click', ".dropdown-item.disability-filter-item", function()
    {
        ...
        if (window.FindLearnerJsBridge)
        {
            window.FindLearnerJsBridge.onFindLearners(keyword, page, disability);
        }
    ...
    .on('click', ".search-item", function() {
        ...
        if (window.FindLearnerJsBridge)
        {
            window.FindLearnerJsBridge.onStalkProfile(id);
        }
    });
    ...
    $('#btn-search').on('click', function()
    {
        ...
        if (window.FindLearnerJsBridge)
        {
            window.FindLearnerJsBridge.onFindLearners(keyword, page, selectedDisabilityFilter);
        }
    });
    
});

function displayLearners(data)
{
    if (IS_FULLY_LOADED)
        showLoading('action');
    ...
    $('#lbl-total-learners').text(obj.total);
    ...
    obj.data.forEach(learner => {

        console.log(`${learner.name} -> ${learner.disability}`)
        let badge = '';

        if (learner.disability > 0)
            badge = `<img src="${disabilityBadges[learner.disability]}" class="ms-auto disability-badge">`;

        let template = `
        <div class="search-item shadow mb-2" data-id="${learner.id}">
            <div class="search-item-avatar-container">
                <img src="${learner.photo}" class="search-item-avatar">
            </div>
            <div class="search-item-details-container overflow-hidden">
                <div class="w-100 d-flex align-items-center gap-3 mb-2">
                    <h6 class="text-truncate search-item-fullname w-100 m-0">${learner.name}</h6>
                    ${badge}
                </div>
                <p class="text-truncate search-item-contact-details w-100">
                    <span class="username">@${learner.username}</span> | ${learner.email}
                </p>
            </div>
        </div>`;
